...
template <typename T, typename CNT, bool padded_cells, size_t ring_size, bool cache_remap>
class PRQueue : public CustomQueueSegmentBase<T, CNT, PRQueue<T, CNT, padded_cells, ring_size, cache_remap>>
{
private:
    using Base = CustomQueueSegmentBase<T, CNT, PRQueue<T, CNT, padded_cells, ring_size, cache_remap>>;
...
public:
    ...
    PRQueue(uint64_t start, int enqThreads, int deqThreads) : Base(enqThreads, deqThreads)
    {
    ...
    static std::string className()
    {
        ...
        return "PRQueue"s + (padded_cells ? "/ca"s : ""s) + (cache_remap ? "/remap" : "") + "/" + CNT::className();
    }
    ...
    bool enqueue(T *item, [[maybe_unused]] const int tid)
    {
        ...
        while (true)
        {
            ...
            uint64_t tailticket = Base::tail.fetch_add(1, tid);
            ...
            if (val == nullptr && nodeIndex(idx) <= tailticket && (!nodeUnsafe(idx) || Base::head.load() <= tailticket))
            {
    ...
    T *dequeue([[maybe_unused]] const int tid)
    {
        ...
        while (true)
        {
            ...
            uint64_t headticket = Base::head.fetch_add(1, tid);
...
template <typename T, typename CNT, bool padded_cells = false, size_t ring_size = 1024, bool cache_remap = true>
using LPRQueue = LinkedRingQueue<T, PRQueue<T, CNT, padded_cells, ring_size, cache_remap>>;
