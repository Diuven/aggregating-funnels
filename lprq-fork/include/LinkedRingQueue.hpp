...
private:
    ...
    const int enqThreads;
    const int deqThreads;
...
public:
    ...
    explicit LinkedRingQueue(int enqThreads, int deqThreads)
        : MetricsAwareBase(enqThreads + deqThreads), maxThreads{enqThreads + deqThreads},
          enqThreads{enqThreads}, deqThreads{deqThreads}
    {
        // Shared object init
        Segment *sentinel = new Segment(0, enqThreads, deqThreads);
    ...
    void enqueue(T *item, int tid)
    {
        ...
        while (true)
        {
            ...
            Segment *newTail = new Segment(ltail->getNextSegmentStartIndex(), enqThreads, deqThreads);
    ...
    void fixState()
    {
        while (true)
        {
            ...
            if (tail.load() != t)
                continue;
            if (h > t)
            { // h would be less than t if queue is closed
                uint64_t tmp = t;
...
public:
    QueueSegmentBase(int enqueue_threads, int dequeue_threads) {}

    friend class LinkedRingQueue<T, Segment>;
};

template <class T, class Segment>
struct SimpleQueueSegmentBase
{
protected:
    SIMPLE_ATOMIC_COUNTER::SimpleAtomicCounter<uint64_t> head;
    SIMPLE_ATOMIC_COUNTER::SimpleAtomicCounter<uint64_t> tail;
    alignas(128) std::atomic<Segment *> next{nullptr};
    ...
    void fixState()
    {
        while (true)
        {
            ...
            { // h would be less than t if queue is closed
                uint64_t tmp = t;
                if (tail.compare_exchange(tmp, h))
                    break;
                continue;
            }
            break;
    ...
    bool closeSegment(const uint64_t tailticket, bool force)
    {
        ...
        {
            uint64_t tmp = tailticket + 1;
            return tail.compare_exchange(tmp, (tailticket + 1) | (1ull << 63));
        }
        else
        {
            return BIT_TEST_AND_SET63(&tail);
...
public:
    SimpleQueueSegmentBase(int enqueue_threads, int dequeue_threads)
        : head(0, dequeue_threads), tail(0, enqueue_threads) {}

    friend class LinkedRingQueue<T, Segment>;
};

template <class T, class CNT, class Segment>
struct CustomQueueSegmentBase
{
protected:
    CNT head;
    CNT tail;
...
public:
    CustomQueueSegmentBase(int enqueue_threads, int dequeue_threads)
        : head(0, dequeue_threads), tail(0, enqueue_threads) {}
